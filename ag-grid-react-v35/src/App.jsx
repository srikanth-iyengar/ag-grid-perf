...
function getCurrentVersion() {
  ...
  return window.location.pathname.match(/-v(\d+)(?:\/|$)/)?.[1] ?? '35';
}

function getVersionHref(nextVersion) {
  return new URL(`../ag-grid-${FRAMEWORK}-v${nextVersion}/`, window.location.href).toString();
}

function attachViewportDatasource(api, datasource) {
  if (typeof api.setGridOption === 'function') {
    api.setGridOption('viewportDatasource', datasource);
    return;
  }

  if (typeof api.setViewportDatasource === 'function') {
...
function MockServerDevtools({ currentVersion, serverConfig, gridConfig, onApply }) {
      ...
      {open ? (
        <div className="devtools-panel">
          ...
          <label className="devtools-field">
            ...
            <select value={currentVersion} onChange={(event) => { window.location.href = getVersionHref(event.target.value); }}>
...
function App() {
  ...
  const onGridReady = ({ api }) => {
    ...
    attachViewportDatasource(api, datasource);
  };
  ...
  return (
    <>
      ...
      <MockServerDevtools currentVersion={currentVersion} serverConfig={serverConfig} gridConfig={gridConfig} onApply={applyConfig} />
    </>
  );
}
